import classNames from "./poolCreate.module.css";
...
const PoolCreate = () => {
    ...
    const { defaultSettings } = useSelector((state) => state.pool);
    ...
    const handleSubmit = useCallback((values, { setSubmitting }) => {
        ...
        dispatch(createPool({ poolData: newPool }))
            .unwrap()
            .then((response) => {
                setSaveStatus("success");

                if (file && response.Pool?.id) {
                    dispatch(uploadPoolImage({ poolID: response.Pool.id, file: file }))
                        .unwrap()
                        .then((uploadResponse) => {
                            setPhotoId(uploadResponse.File?.id);
                            setPhotoUrl(uploadResponse.File?.url);
                            console.log('Image uploaded successfully after pool creation');
                        })
                        .catch((uploadError) => {
                            console.error('Error uploading image after pool creation:', uploadError);
                        });
                }

                setTimeout(() => {
                    setSaveStatus(null);
                    navigate(`/pool-details?id=${response.Pool.id}`);
                }, 2000);
            })
            .catch((error) => {
                setSaveStatus("error");
    ...
    }, [dispatch, photoId, currentUser, navigate, file]);
    ...
    const handleFileChange = (e) => {
        ...
        if (selectedFile) {
            ...
            // For pool creation, we'll store the file and upload it after pool is created
            ...
            <div className="row">
                <div className="col-lg-8">
                    <div className="glass rounded-2xl border-soft p-4 shadow-soft mb-4">
                        <h1 className="text-3xl fw-extrabold tracking-tight">Create New Pool</h1>
                        <br></br>
                        <Formik
                            initialValues={{
                                name: "",
                                description: "",
                                goal_amount: 100,
                                defaultBuy_in_amount: 10,
                                is_private: true,
                                // Voting settings defaults
                                        ...
                                        <div>
                                            {saveStatus === "success" && (
                                                <div className="text-success fw-semibold">
                                                    <i className="bi bi-check-circle-fill me-1"></i> Pool created successfully
                                                </div>
                                            )}
                                            {saveStatus === "error" && (
                                                <div className="text-danger fw-semibold">
                                                    <i className="bi bi-exclamation-circle-fill me-1"></i> Error creating pool
                                                </div>
                                            )}
                                        </div>
                                        <button
                                            ...
                                            disabled={isSubmitting || creatingPool}
                                        >
                                            {isSubmitting || creatingPool ? (
                                                <>
                                                ...
                                                <>
                                                    <i className="bi bi-plus-lg me-1"></i> Create Pool
                                                </>
                                            )}
                                        </button>
                                    </div>
                ...
                <div className="col-lg-4">
                    <div className="glass rounded-2xl border-soft p-4 shadow-soft mb-4">
                        ...
                        <div className="text-center mb-3">
                            <img
                                src={photoUrl || defaultSettings?.pool?.pool_image}
                                alt="Pool"
                        ...
                        {imageUploadStatus === "pending" && (
                            <div className="alert alert-info py-2 mb-3" role="alert">
                                <i className="bi bi-info-circle-fill me-1"></i> Image will be uploaded after pool creation
                            </div>
                        )}
                    ...
                    <div className="glass rounded-2xl border-soft p-4 shadow-soft">
                        <h5 className="fw-semibold d-flex align-items-center gap-2 mb-3">
                            <i className="bi bi-info-circle text-primary"></i> Creating a Pool
                        </h5>
                        <ul className="list-unstyled text-muted small">
                            <li className="mb-2"><i className="bi bi-check-lg text-success me-1"></i> Set a clear goal amount for your pool</li>
                            <li className="mb-2"><i className="bi bi-check-lg text-success me-1"></i> Choose an appropriate default buy-in amount</li>
                            <li className="mb-2"><i className="bi bi-check-lg text-success me-1"></i> Enable voting for important decisions</li>
                            <li className="mb-2"><i className="bi bi-check-lg text-success me-1"></i> Add deadlines to keep your pool on track</li>
                            <li className="mb-2"><i className="bi bi-check-lg text-success me-1"></i> Make it private if you want to control membership</li>
                        </ul>
                    </div>
                </div>
            </div>
