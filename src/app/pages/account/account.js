...
import {
    LuCamera, LuShare2, LuSettings, LuLock, LuShieldCheck, LuBell, LuGlobe, LuChevronRight, LuMail
} from 'react-icons/lu';
import {
    getAccount,
    updateProfile,
...
const Account = () => {
    ...
    const photoUrl = profile?.photoUrl || 'https://app.collectly.com/public/img/user.png';
    ...
    return (
        <>
            <div className={styles.accountPage}>
                ...
                <div className={styles.profileHeader}>
                    <div className={styles.avatarWrapper}>
                        ...
                        />
                        <button className={styles.avatarEditBtn} onClick={handlePhotoClick} aria-label="Change profile picture">
                            <LuCamera size={18} />
                        </button>
                    </div>
                    ...
                    <div className={styles.profileInfo}>
                        ...
                        <div className={styles.profileBadges}>
                            <span className={styles.badgePro}>Pro Member</span>
                            <span className={styles.badgeVerified}>Verified</span>
                        </div>
                    </div>

                    <div className={styles.profileActions}>
                        <button className={styles.iconBtn} aria-label="Share profile">
                            <LuShare2 size={18} />
                        </button>
                        <button className={styles.iconBtn} aria-label="Settings" onClick={() => setActiveTab('settings')}>
                            <LuSettings size={18} />
                        </button>
                    </div>
                </div>
                ...
                <div className={styles.mainGrid}>
                    ...
                    <div className={styles.leftCol}>
                        ...
                        <div className={styles.acCard}>
                            <div className={styles.cardHeader}>
                                ...
                                <button className={styles.editAllBtn} onClick={() => setIsEditing(!isEditing)}>
                            ...
                            {!isEditing ? (
                                <div className={styles.fieldsGrid}>
                                    {[
                                        ...
                                        { label: 'Date of Birth', value: profile?.date_of_birth ? moment(profile.date_of_birth).format('MMM D, YYYY') : '—' },
                                        { label: 'Member Since', value: profile?.createdAt ? moment(profile.createdAt).format('MMM YYYY') : '—' },
                                    ].map((f, i) => (
                                        <div key={i} className={styles.fieldBox}>
                                    ...
                                    {({ errors, values, setFieldValue, touched, isSubmitting, handleSubmit }) => (
                                        <Form onSubmit={handleSubmit}>
                                            ...
                                            <div className="d-flex gap-3">
                                                ...
                                                <Button type="button" variant="outline-secondary" size="lg" onClick={() => setIsEditing(false)}>
                                                    Cancel
                                                </Button>
                                            </div>
                                        </Form>
                        ...
                        {/* Community Analytics */}
                        <div className={styles.acCard}>
                            <h2 className={styles.cardTitle} style={{ marginBottom: 24 }}>Community Analytics</h2>
                            <div className={styles.statsGrid}>
                                <div className={styles.statBox}>
                                    <p className={styles.statNumber}>{profile?.pools_created || 0}</p>
                                    <p className={styles.statLabel}>Pools Created</p>
                                </div>
                                <div className={styles.statBox}>
                                    <p className={styles.statNumber}>{contributions?.pagination?.totalItems || 0}</p>
                                    <p className={styles.statLabel}>Supports Given</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className={styles.rightCol}>
                        ...
                        <div className={styles.safetyCard}>
                            <h3 className={styles.safetyTitle}>Account Safety</h3>
                            <div className={styles.safetyActions}>
                                {[
                                    { Icon: LuLock, label: 'Change Password', tab: 'password' },
                                    { Icon: LuShieldCheck, label: 'Two-Factor Auth', tab: null },
                                    { Icon: LuBell, label: 'Notification Settings', tab: null },
                                    { Icon: LuGlobe, label: 'Privacy Settings', tab: 'settings' },
                                ].map((a, i) => (
                                    <button
                                        key={i}
                                        ...
                                        disabled={!a.tab}
                                    >
                                        <span className={styles.safetyBtnLeft}>
                                            <a.Icon size={17} />
                                            <span>{a.label}</span>
                                        </span>
                        ...
                        <div className={styles.dangerCard}>
                            ...
                            <p className={styles.dangerText}>
                                Permanent account deletion removes all your data, contributions, and history. This action cannot be undone.
                            </p>
                            <button className={styles.deleteBtn} onClick={() => setActiveTab('close')}>
                                Delete Account
                            </button>
                        </div>
            ...
            <Modal size="md" aria-labelledby="contained-modal-title-vcenter" centered show={showAppModal} onHide={() => setShowAppModal(false)}>
                <Modal.Header closeButton>
                    <Modal.Title id="contained-modal-title-vcenter">Create New Project</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Formik
                        initialValues={{ name: '', redirectUri: '' }}
                        validationSchema={Yup.object().shape({
                        ...
                        {({ errors, touched, isSubmitting, handleSubmit }) => (
                            <Form onSubmit={handleSubmit}>
                                <Row>
                                    <Col className="mb-3">
                                        <Form.Group>
                                            <Form.Label className="fw-semibold">Name Your Project</Form.Label>
                                            <Field name="name" type="text" placeholder="Enter project name" className={`form-control ${errors.name && touched.name ? "is-invalid" : ""}`} />
                                            <ErrorMessage name="name" component="div" className="invalid-feedback" />
                                            <Form.Text className="text-muted">Choose a descriptive name for your API project</Form.Text>
                                        </Form.Group>
                                    </Col>
                                </Row>
                                <Row>
                                    <Col className="mb-3">
                                        <Form.Group>
                                            <Form.Label className="fw-semibold">Redirect URI (optional)</Form.Label>
                                            <Field name="redirectUri" type="url" placeholder="https://example.com/callback" className={`form-control ${errors.redirectUri && touched.redirectUri ? "is-invalid" : ""}`} />
                                            <ErrorMessage name="redirectUri" component="div" className="invalid-feedback" />
                                            <Form.Text className="text-muted">Optional. Add an OAuth redirect/callback URL if applicable.</Form.Text>
                                        </Form.Group>
                                    </Col>
                                </Row>
                                <div className="d-flex gap-2 justify-content-end">
                                    <Button variant="outline-secondary" onClick={() => setShowAppModal(false)} disabled={isSubmitting}>Cancel</Button>
                                    <Button type="submit" variant="primary" disabled={isSubmitting} className="px-4">
                                        {isSubmitting ? (<><Spinner animation="border" size="sm" className="me-2" />Creating...</>) : 'Create Project'}
                                    </Button>
                                </div>
                            </Form>
                        )}
            ...
            <Modal show={showPhotoModal} onHide={handleClosePhotoModal} centered size="md">
                <Modal.Header closeButton>
                    <Modal.Title><i className="bi bi-camera me-2"></i>Update Profile Photo</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <div className="text-center">
                        <div className="mb-4">
                            ...
                            <small className="text-muted">{previewUrl ? 'New photo preview' : 'Current profile photo'}</small>
                        </div>
                        <Form.Group className="mb-4">
                            <Form.Label className="fw-semibold">Choose a new photo</Form.Label>
                            <Form.Control type="file" accept="image/jpeg, image/jpg, image/png, image/gif" onChange={handleFileSelect} className="mb-2" />
                            <Form.Text className="text-muted">Supported formats: JPEG, PNG, GIF. Max size: 5MB</Form.Text>
                        </Form.Group>
                        <div className="d-grid gap-2">
                            <Button variant="primary" onClick={handlePhotoUpload} disabled={!selectedFile || isUploading} size="lg">
                                {isUploading ? (<><Spinner animation="border" size="sm" className="me-2" />Uploading...</>) : (<><i className="bi bi-upload me-2"></i>Upload Photo</>)}
                            </Button>
                            {profile?.photoUrl && !previewUrl && (
                                <Button variant="outline-danger" onClick={handleRemovePhoto} disabled={isUploading}>
                                    <i className="bi bi-trash me-2"></i>Remove Current Photo
                                </Button>
                            )}
                            <Button variant="outline-secondary" onClick={handleClosePhotoModal} disabled={isUploading}>Cancel</Button>
                        </div>
                    </div>
                </Modal.Body>
            </Modal>
